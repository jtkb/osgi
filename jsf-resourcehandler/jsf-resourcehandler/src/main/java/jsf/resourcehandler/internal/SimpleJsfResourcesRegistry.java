...
public class SimpleJsfResourcesRegistry implements JsfResourcesRegistry {
	...
	public void register(Bundle bundle) {
		synchronized (activated) {
			...
			log.log(LogService.LOG_INFO,
					String.format("Registering JSF-Resource-Bundle: %s", bundle.getSymbolicName()));
	...
	public void unregister(Bundle bundle) {
		synchronized (activated) {
			if (!activated.contains(bundle))
				return;
			log.log(LogService.LOG_INFO,
					String.format("De-Registering JSF-Resource-Bundle: %s", bundle.getSymbolicName()));
	...
	/**
	 * Searches all registered bundles for a given resource. The first match
	 * will be created as javax.faces.application.Resource. In order to conform
	 * to the Servlet 3.0 resources, the lookup will start at
	 * META-INF/resources/
	 * 
	 * @param resourceName
	 *            name or path of the resource to create
	 * @param libraryName
	 * @return Resource for given resource
	 */
	public Resource createResource(String resourceName, String libraryName) {
		if (resourceName == null) {
			log.log(LogService.LOG_ERROR, "Method called with invalid argument!",
		...
		if (libraryName.charAt(0) == '/') {
			// If libraryName starts with '/', remove that character because it
			// does not have any meaning (with and without should point to the
			// same resource).
			libraryName = libraryName.substring(1);
		}
		final StringBuilder lookupString = new StringBuilder(JSF_DEFAULT_RESOURCE_FOLDER);
		if (libraryName != null)
			lookupString.append(libraryName).append('/');
		lookupString.append(resourceName);

		System.out.println("=================== SEARCHING: " + lookupString.toString() + " in " + activated.size() + " bundles.");
		Optional<Bundle> searchResult = activated.stream().filter(b -> b.getResource(lookupString.toString()) != null)
				.findFirst();
		if (searchResult.isPresent()) {
			return new JsfOsgiResource(searchResult.get().getResource(lookupString.toString()), resourceName, null);
		}
		return null;
	}

	public ViewResource createResource(String resourceName) {
		if (resourceName == null) {
			log.log(LogService.LOG_ERROR, "Method called with invalid argument!",
					new IllegalArgumentException("resourceName for lookup must not be null!"));
		}
